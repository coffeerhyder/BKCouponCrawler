...
def coupon2FixProductTitle(productTitle: str) -> str:
    ...
    fries = re.compile("(?i)((\\d\\s*)?king\\s*fries\\s*(small|large|medium)).*").search(newProductTitle)
    if fries:
    ...
    dipQuantifier = re.compile("(?i)(\\s*?\\+\\s*(\\d+)\\s*(DIPs?))").search(productTitle)
    if dipQuantifier:
    ...
    forPriceOfOne = re.compile("(?i)\\s*?ZUM\\s*PREIS\\s*VON\\s*EINER(\\s*\\d+ER\\s*PORTION)?").search(newProductTitle)
    if forPriceOfOne:
        newProductTitle = newProductTitle.replace(forPriceOfOne.group(0), '')
    buyOneGetOneFree = re.compile("(?i)[\\s-]*?Buy\\s*1\\s*get\\s*1\\s*free").search(newProductTitle)
    if buyOneGetOneFree:
    ...
    whopperJrFix = re.compile("(?i)((\\d+)\\s*)?WHOPPER\\s*JR\\.?").search(newProductTitle)
    if whopperJrFix:
    ...
    longTexasFix = re.compile("(?i)((\\d+)\\s*)?LONG\\s*TEXAS\\s*BBQ\\s*31611").search(newProductTitle)
    if longTexasFix:
    ...
    nuggetsFix1 = re.compile("(?i)KING\\s*NUGGETS(?:\\s*®)?\\s*(\\d+)\\s*STK\\s*\\.?").search(newProductTitle)
    if nuggetsFix1:
    ...
    nuggetsFix2 = re.compile("(?i)CHICKEN\\s*NUGGETS\\s*\\((\\d+)\\)").search(newProductTitle)
    if nuggetsFix2:
