...
from Helper import *
from Helper import getPathImagesOffers, \
    isValidImageFile, CouponType, Paths
from utils.Config import Config
from utils.UtilsCouponsDB import getCouponTitleMapping
...
class BKCrawler:
    ...
    def __init__(self, cfg: Config = None, db: DBManager = None, allowAddExtraCouponsOnInit: bool = True):
        ...
        self.updateCaches()
        self.updateCachedMissingPaperCouponsInfo()
    ...
    def updateCaches(self):
        ...
        # Get all coupons through the DB manager
        coupons = self.db.get_coupons()
        ...
        for coupon in coupons:
            if coupon.isValid():
                category = newCachedAvailableCouponCategories.setdefault(
                    coupon.type,
                    CouponCategory(coupons=coupon.type)
                )
        ...
        self.cachedFutureCoupons = sorted(
            futureCoupons,
            key=lambda x: 0 if x.getStartDatetime() is None else x.getStartDatetime().timestamp()
        )

        # Update missing paper coupons info
        self.updateCachedMissingPaperCouponsInfo()

        # Generate future coupons text
    ...
    def updateCachedMissingPaperCouponsInfo(self):
        """
        ...
        # Get valid paper coupons using the filter functionality
        validPaperCoupons = self.db.get_filtered_coupons_as_list(
            CouponFilter(
                allowedCouponTypes=[CouponType.PAPER],
                activeOnly=True
            )
        )

        # Group coupons by expiration date
        paperCouponMapping = {}
        for coupon in validPaperCoupons:
            expireDate = coupon.getExpireDateFormatted()
            if expireDate not in paperCouponMapping:
                paperCouponMapping[expireDate] = []
            paperCouponMapping[expireDate].append(coupon)
    ...
    def addCouponsToDB(self, couponsToAddToDB: Union[dict, List[Coupon]]) -> bool:
        if len(couponsToAddToDB) == 0:
            # Nothing to do
            return False
        if isinstance(couponsToAddToDB, dict):
            couponsToAddToDB = list(couponsToAddToDB.values())
        allCoupons = self.db.get_coupons()
        infoDBDoc = self.db.get_info_entry()
        ...
        for crawledCoupon in couponsToAddToDB:
            existingCoupon = self.db.get_coupon(crawledCoupon.id)
            # Update DB
    ...
    async def crawl(self):
        ...
        if apiCrawlerException is not None:
            # Raise exception to signal upper handling that API crawler has failed -> "Fail gracefully"
    ...
    async def crawlCoupons(self, crawledCouponsDict: dict):
        ...
        headers = {"Origin": "https://www.burgerking.de",
        ...
        headers.update(userAgentHeaders)
    ...
    async def addExtraCoupons(self, crawledCouponsDict: dict, immediatelyAddToDB: bool):
        ...
        if immediatelyAddToDB and len(extraCouponsToAdd) > 0:
            # Add items to DB
            dbWasUpdated = self.addCouponsToDB(couponsToAddToDB=extraCouponsToAdd)
            if dbWasUpdated:
                # Important!
                await self.downloadProductiveCouponDBImagesAndCreateQRCodes()
    ...
    def getValidExtraCoupons(self) -> dict:
        ...
        if papercouponlist is not None and len(papercouponlist) > 0:
            logging.info(f"Number of valid paper coupons [active + coming soon]: {len(papercouponlist)}")
    ...
    def processCrawledCoupons(self, crawledCouponsDict: dict):
        ...
        self.addCouponsToDB(couponsToAddToDB=couponsToAddToDB)
        ...
        if len(deleteCouponDocs) > 0:
            self.db.delete_coupon(list(deleteCouponDocs.values()))

        logging.info(f"Coupons deleted: {len(deleteCouponDocs)}")
        if len(deleteCouponDocs) > 0:
            logging.info(f"Coupons deleted IDs: {list(deleteCouponDocs.keys())}")
        logging.info(f"Coupon processing done | Total number of coupons in DB: {len(coupons) - len(deleteCouponDocs)}")
    ...
    def couponCsvExport(self):
        ...
        # Get all coupons through the DBManager
        coupons = self.db.get_coupons()
        ...
        with open('crawler/coupons.csv', 'w', newline='') as csvfile:
            ...
            for coupon in coupons:
                writer.writerow({
                    'PRODUCT': coupon.getTitle(),
                    'MENU': coupon.isContainsFriesAndDrink(),
                    'PLU': coupon.plu if coupon.plu is not None else "N/A",
                    'PLU2': coupon.id,
                    'TYPE': coupon.type,
                    'PRICE': coupon.get(Coupon.price.name, -1),
                    'PRICE_COMPARE': coupon.get(Coupon.priceCompare.name, -1),
    ...
    def couponCsvExport2(self):
        ...
        with open('crawler/coupons2.csv', 'w', newline='') as csvfile:
            ...
            # Get all coupons through the DB manager
            coupons = self.db.get_filtered_coupons_as_list(
                CouponFilter(allowedCouponTypes=[CouponType.PAPER], activeOnly=False)
            )

            for coupon in coupons:
                writer.writerow({
                    'Produkt': coupon.getTitle(),
                    'Menü': coupon.isContainsFriesAndDrink(),
                    'PLU': coupon.plu,
                    'PLU2': coupon.id,
                    'Preis': coupon.getPrice(),
                    'OPreis': coupon.get(Coupon.priceCompare.name, -1),
...
if __name__ == '__main__':
    crawler = BKCrawler()
    crawler.exportCSVs = False

    # crawler.setExportCSVs(True)
    # crawler.setCrawlOnlyBotCompatibleCoupons(False)
    print("Number of userIDs in DB: " + str(len(crawler.db.get_user_db())))
