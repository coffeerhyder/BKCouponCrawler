...
from UtilsCouponsDB import couponDBGetUniqueIdentifier, Coupon, User, ChannelCoupon, InfoEntry, CouponSortMode
from CouponCategory import CouponCategory, BotAllowedCouponSources
...
def updatePublicChannel(bkbot, updateMode: ChannelUpdateMode):
    ...
    if len(couponsToSendOut) > 0:
        ...
        for coupon in couponsToSendOut.values():
            ...
            couponText = bkbot.getCouponLongText(coupon)
    ...
    if len(updatedCoupons) > 0 or len(deletedChannelCoupons) > 0 or len(
        ...
        """  Sort coupons: Send the different types separately. """
        couponsSeparatedByType = {}
        for couponSource in BotAllowedCouponSources:
            couponsTmp = list(filter(lambda x: x[Coupon.source.name] == couponSource, list(activeCoupons.values())))
                    ...
                    for couponIndex in range(startIndex, startIndex + maxCouponsPerPage):
                        ...
                        if coupon.id in channelDB:
                            ...
                            if len(channelCoupon.messageIDs) > 0:
                                couponText = bkbot.generateCouponShortTextWithHyperlinkToChannelPost(coupon, channelCoupon.messageIDs[0])
                            else:
...
def editMessageAndWait(bkbot, messageID: Union[int, None], messageText) -> bool:
    ...
    if messageID is None:
        return False
    try:
        bkbot.updater.bot.edit_message_text(chat_id='@' + bkbot.getPublicChannelName(), message_id=messageID, text=messageText, parse_mode="HTML", disable_web_page_preview=True)
        return True
    except BadRequest:
